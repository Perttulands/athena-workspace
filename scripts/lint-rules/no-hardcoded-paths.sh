...
# Look for hardcoded paths like /home/perttu
LINE_NUM=0
while IFS= read -r line; do
    LINE_NUM=$((LINE_NUM + 1))

    # Check for /home/perttu specifically
    if echo "$line" | grep -qE '"/home/perttu[^"]*"'; then
        # Found hardcoded path
        jq -n \
            --arg rule "no-hardcoded-paths" \
            --arg file "$FILE" \
            --argjson line "$LINE_NUM" \
            --arg message "Hardcoded absolute path /home/perttu found" \
            --arg fix "Use \$HOME or relative paths instead of hardcoded /home/perttu" \
