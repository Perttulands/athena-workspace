...
The server runs ext4 on a single disk. No SSD TRIM issues, no NFS gotchas, no distributed filesystem concerns. The PRD's filesystem assumptions are valid for this deployment. If the system ever moves to a shared filesystem (NFS, CIFS), `flock` semantics change and `O_CREAT|O_EXCL` may not be atomic. Document this as a hard constraint: **relay requires a local POSIX filesystem**.
