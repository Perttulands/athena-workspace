...
> Produced: 2026-02-16  
> Version analyzed: 0.3.0 (local clone at `$HOME/mcp_agent_mail/`)  
...
Type=simple
User={{USERNAME}}
WorkingDirectory=$HOME/mcp_agent_mail
EnvironmentFile=$HOME/mcp_agent_mail/.env
ExecStart=$HOME/.local/bin/uv run python -m mcp_agent_mail.cli serve-http --host 127.0.0.1 --port 8765
Restart=on-failure
RestartSec=5
